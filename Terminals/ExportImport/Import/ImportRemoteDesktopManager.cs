...
namespace Terminals.ExportImport.Import
{
    public class ImportRemoteDesktopManager : IImport
    {
        ...
        Dictionary<string, Configuration.Files.Credentials.CredentialSet> resolvedCredentials = new Dictionary<string, Configuration.Files.Credentials.CredentialSet>();
            ...
            using (FileStream fileStream = new FileStream(fileName, FileMode.Open))
            {
                ...
                foreach (RemoteDesktopManager.Connection connection in result.Connections)
                {
                    if (connection.ConnectionType == "Credential")
                    {
                        ...
                        {
                            Configuration.Files.Credentials.CredentialSet credentialSet = Configuration.Files.Credentials.StoredCredentials.GetKeePassCredentialById(connection.Credentials.KeepassUuid);
                            ...
                            if (credentialSet != null)
                            {
                                ...
                                if (string.IsNullOrWhiteSpace(Configuration.Files.Main.Settings.Settings.KeePassPath) || string.IsNullOrWhiteSpace(Configuration.Files.Main.Settings.Settings.KeePassPassword))
                            {
